...
def monitor_network(pcap_file, interval):
    ...
    while not stop_monitoring.is_set():
        ...
        result = df.to_json(orient="records")
        print("Sending result data:", result)  # Debugging line
        is_intrusion = lgbm_inference(
            pd.concat([df, pd.DataFrame({"label": [0] * len(df)})], axis=1),
            encoder,
            scaler,
        )
        df_result = pd.concat(
            [df, pd.DataFrame({"is_intrusion": is_intrusion})], axis=1
        ).to_json(orient="records")

        print("Emitting monitor_result:", df_result)  # Debugging line
        socketio.emit("monitor_result", df_result)
...
@data_bp.route("/start_monitor", methods=["POST"])
def start_monitor():
    global monitoring_thread, stop_monitoring, duration
    ...
    monitoring_thread = threading.Thread(
        target=monitor_network, args=(pcap_file, duration)
    )
    monitoring_thread.start()
...
@data_bp.route("/stop_monitor", methods=["POST"])
def stop_monitor():
    global stop_monitoring, monitoring_thread
